...
res = ['TF+','TD+','TO+']
folderRes = ['../res/evodag-0.10.6/TF+/','../res/evodag-0.10.6/TD+/','../res/evodag-0.10.6/TO+/']
folderData = '../data/'
    ...
    for k in range(len(res)):
        ...
        for j in range(1,datasets_size[i]+1):
            ...
            with gzip.open(fileModel, 'r') as fpt:
                m = pickle.load(fpt)
                fitness.append( m.fitness_vs )
        fitness_v = numpy.median( numpy.array(fitness) )
        strv += '& '+str( round(fitness_v,4))
    print(strv + '\\\\ \\hline')
print(' ')
